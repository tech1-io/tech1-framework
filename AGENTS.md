...
- **CI** (`.github/workflows/main.yml`): on push to `main`, runs `./mvnw clean install` on Java 21 (Temurin). Maven `deploy` to GitHub Packages and the Docker image pushes (`jbst-server-iam`, `jbst-server-resource-burner` → `ghcr.io`) are gated behind the `MAVEN_DEPLOYMENT_ENABLED` / `DOCKER_PUSH_ENABLED` flags in **`.github/deployment.env`** (loaded into `GITHUB_ENV` by the workflow; normally `false` on snapshots). The flags and `DOCKER_VERSION` live in that env file — not in `main.yml` — because the release workflow's `GITHUB_TOKEN` is not allowed to push commits that modify `.github/workflows/*`. These flag-gated docker steps only fire on human-pushed release commits — the one-click release pushes with `GITHUB_TOKEN`, which never triggers `main.yml`, so `release.yml` pushes the images itself.
...
- **One-click release** (`.github/workflows/release.yml`, Actions → `release` → *Run workflow*): automates the whole cadence on a runner — guards (version must be `-SNAPSHOT`, tag must not exist, `CHANGELOG.md` must not contain `— TBD`), extracts release notes from `CHANGELOG.md`, runs `next-release.sh`, deploys with `./mvnw clean -DskipTests -Dmaven.test.skip deploy -Pgithub` (no tests, same as the `main.yml` deploy step — `main` is assumed green from regular CI; a failed build still aborts before anything is pushed), builds and pushes the `jbst-server-iam` and `jbst-server-resource-burner` Docker images to `ghcr.io` tagged with the release version, commits/pushes `RELEASE: vX.Y`, creates GitHub release + tag `vX.Y` with the notes, then runs `next-snapshot.sh` and commits/pushes `SNAPSHOT: vX.(Y+1)`. The two bot commits do **not** trigger `main.yml` (GITHUB_TOKEN pushes don't start workflows) — deployment already happened inside the release run.
